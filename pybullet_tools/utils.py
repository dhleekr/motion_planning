...
from motion.motion_planners.meta import direct_path, solve
...
def get_sample_fn(body, joints, custom_limits={}, **kwargs):
    ...
    lower_limits, upper_limits = [-3.0, -3.0], [3.0, 3.0]
...
def check_initial_end(start_conf, end_conf, kappa, phi, collision_fn, verbose=False):
    # TODO: collision_fn might not accept kwargs
    if collision_fn(start_conf, verbose=verbose):
        print('Warning: initial configuration is in collision')
        print(start_conf)
        # idx1 = round((c_space.shape[0]-1)/6*(3-start_conf[1]))
        # idx2 = round((c_space.shape[1]-1)/6*(3+start_conf[0]))
        # c_space[idx1][idx2] = 1
        kappa.append(start_conf)
        phi.append(1)
        return False
    if collision_fn(end_conf, verbose=verbose):
        print('Warning: end configuration is in collision')
        print(end_conf)
        # idx1 = round((c_space.shape[0]-1)/6*(3-end_conf[1]))
        # idx2 = round((c_space.shape[1]-1)/6*(3+end_conf[0]))
        # c_space[idx1][idx2] = 1
        kappa.append(end_conf)
        phi.append(1)
        return False
    return True

def plan_joint_motion(body, joints, end_conf, kappa, phi, obstacles=[], attachments=[],
                      self_collisions=True, disabled_collisions=set(),
    ...
    collision_fn = get_collision_fn(body, joints, obstacles, attachments, self_collisions, disabled_collisions,
                                    ...
                                    use_aabb=use_aabb, cache=cache)
    start_conf = get_joint_positions(body, joints)
    if not check_initial_end(start_conf, end_conf, kappa, phi, collision_fn):
        return None, None
    if algorithm is None:
        return rrt(start_conf, end_conf, kappa, phi, distance_fn, sample_fn, extend_fn, collision_fn, **kwargs)
...
def plan_lazy_prm(start_conf, end_conf, sample_fn, extend_fn, collision_fn, **kwargs):
    ...
    from motion.motion_planners.lazy_prm import lazy_prm
